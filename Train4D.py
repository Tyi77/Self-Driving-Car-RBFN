...
def get_trace(fArea, rSpace, rbfs, w):
    ...
    status_start = [0, 0, 90] # [x, y, wheelAngle]
    dist_start = [22., 8.4853, 8.4853] # [forward_dist, right45_dist, left45_dist]
    b = 3 # car size
    ...
    while True:
        ...
        # Change dist
        dist = detect_dist(status, space)

        tmp = np.concatenate(((status[0], status[1]), dist, [theta], [status[2]]))
...
def plot_trace(*args):
    ...
    rbfList, weights = API.rbfn_train('train4DAll.txt', _hidden_dim, _identity_coefficient, _sigma)
    ...
    # ---Output the Trace---
    output = [[p[2], p[3], p[4], p[5]] for p in trace]
    np.savetxt('carTrace4D.txt', output, fmt='%06.4f')
...
def set_frame_content():
    ...
    # root = API.root
    train4D_frame = API.train4D_frame
    _font = API._font()

    # ---Widgets Setting---
    title = Label(train4D_frame, text='Train 4D', font=_font.mjh34b)

    playground_fig = Figure(figsize=(10, 10))
    playground_canvas = FigureCanvasTkAgg(playground_fig, master=train4D_frame)

    forward_dist_lb = Label(train4D_frame, text='Forward Dist', font=_font.mjh12)
    forward_dist = Label(train4D_frame, text='', font=_font.mjh12)
    right45_dist_lb = Label(train4D_frame, text='Right45 Dist', font=_font.mjh12)
    right45_dist = Label(train4D_frame, text='', font=_font.mjh12)
    left45_dist_lb = Label(train4D_frame, text='Left45 Dist', font=_font.mjh12)
    left45_dist = Label(train4D_frame, text='', font=_font.mjh12)

    argument_area = LabelFrame(train4D_frame, relief='ridge', borderwidth=2)

    # hidden_dim_lb = Label(train4D_frame, text='Hidden\nDimension', font=_font.mjh12)
    # hidden_dim = Entry(train4D_frame, font=_font.mjh12)

    # identityC_lb = Label(train4D_frame, text='Identity\nCoefficient', font=_font.mjh12)
    # identityC = Spinbox(train4D_frame, from_=0.0000001, to= 0.1, increment=0.0000001, font=_font.mjh12)

    sigma_lb = Label(train4D_frame, text='Sigma', font=_font.mjh12)
    # sigma = Scale(train4D_frame, from_=0.0, to=1.0, resolution=0.00001, tickinterval=0.5, orient='horizontal', cursor='cross')
    sigma = Spinbox(train4D_frame, from_=0.000000001, to= 0.1, increment=0.00000001, font=_font.mjh12)
    ...
    submit_bt = Button(train4D_frame, text='Run!', font=_font.mjh12, command=lambda: plot_trace(*args))

    # clear_bt = Button(train4D_frame, text='Clear!', font=_font.mjh12, command=lambda: clear(*[playground_fig]))
